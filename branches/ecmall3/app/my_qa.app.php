...
class My_qaApp extends StoreadminbaseApp
{
    ...
    function index()
    {
        ...
        $my_qa_data = $this->my_qa_mod->find(array(
            'fields' => 'ques_id,question_content,reply_content,goods_qa.user_id,goods_qa.email,time_post,time_reply,user_name,goods_qa.goods_id,goods_name',
            'join' => 'belongs_to_store,belongs_to_user,belongs_to_goods',
    ...
    function reply()
    {
        if (!IS_POST)
        {
            ...
            $my_qa_data = $this->my_qa_mod->get(array(
                'fields' => 'question_content,reply_content,goods_qa.user_id,goods_qa.email,time_post,user_name,goods_qa.goods_id,goods_name',
                'join' => 'belongs_to_store,belongs_to_user,belongs_to_goods',
                'conditions' => '1=1 '.$conditions,
            ));
        ...
        }else
        {
            ...
            $user_info = $this->my_qa_mod->get(array(
                    ...
                    'fields' => 'user_id,email,g.goods_id,goods_name'));
                extract($user_info);
                $data = array(
