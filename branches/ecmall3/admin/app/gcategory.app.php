...
class GcategoryApp extends BackendApp
{
    ...
    function add()
    {
        ...
        else
        {
            ...
            //vdump($this->_gcategory_mod->info());exit;
            /* 检查名称是否已存在 */
    ...
    function batch_edit()
    {
        ...
        else
        {
            ...
            if ($_POST['parent_id'])
            {
                ...
                // 过滤掉该分类及其祖先分类（因为这些分类不能以该分类为上级分类）
                $parents = array();
                $this->_gcategory_mod->get_parents($parents, $_POST['parent_id']);
                $filter_ids = array();
                foreach ($parents as $cate)
                {
                    $filter_ids[] = $cate['cate_id'];
                }
                $this->_gcategory_mod->edit(array_diff($ids, $filter_ids), $data);
            }
