        ...
        else
        {
            ...
            if ($old_info['state'] != $data['state'])
            {
                ...
                if ($data['state'] == STORE_CLOSED)
                {
                    ...
                    $content = sprintf(Lang::get('your_store_is_closed'), $data['close_reason']);
                }
                else
                {
                    // 开启店铺
                    $subject = Lang::get('open_store_notice');
                    $content = Lang::get('your_store_is_opened');
                }
                $ms->pm->send(MSG_SYSTEM, $old_info['store_id'], $subject, $content);
   ...
   function ajax_col()
   {
    ...
    function drop()
    {
        ...
        foreach ($users as $user)
        {
            ...
            $content = Lang::get('your_store_is_droped');
    ...
    function view()
    {
        ...
        else
        {
            ...
            elseif (isset($_POST['reject']))
            {
                ...
                $title = LANG::get('reject_title');
                $content = sprintf(LANG::get('reject_content'), $reject_reason);
                $ms =& ms();
                $ms->pm->send(MSG_SYSTEM, $id, $title, $content);
