   ...
   function ajax_col()
   {
    ...
    function drop()
    {
        ...
        else
        {
            ...
            {
                $title = LANG::get('drop_goods_notice');
                $content = sprintf(LANG::get('admin_drop_your_goods'), addslashes($goods['goods_name']));
                if (trim($_POST['drop_reason']))
                {
                    $content .= "\r\n" . sprintf(LANG::get('reason_is'), trim($_POST['drop_reason']));
                }
                $ms->pm->send(MSG_SYSTEM, $goods['store_id'], $title, $content);
            }
