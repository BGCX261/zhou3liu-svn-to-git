...
{
    var $_user_dir;     // 设置用户模板存放路径
    var $_default_dir;  // 设置默认模板存放路径
    ...
    {
        $this->_default_dir = ROOT_PATH . '/includes/arrayfiles/mailtemplate/';
        $this->_user_dir    = ROOT_PATH . '/data/mailtemplate/';
    }
    /**
     * 获取默认设置
     *
    ...
    function get_default()
    {
        return array(
            ...
            'touser_find_password' => array(
                'description' =>
                Lang::get('touser_find_password'),
            ),
            'tobuyer_question_replied' => array(
                'description' =>
                Lang::get('tobuyer_question_replied'),
            ),
        );
    }
    function getOne($key)
    {
        ...
        $details_data = file_exists($this->_user_dir . $key . '.php') ? include($this->_user_dir . $key . '.php') : include($this->_default_dir . $key . '.php');
