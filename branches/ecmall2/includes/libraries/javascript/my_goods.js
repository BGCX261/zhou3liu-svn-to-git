...
function spec_update(){
    ...
    if(SPEC.spec_name_1){
        spec_name_1.show();
        spec_name_1.html(SPEC.spec_name_1 + '<input type="hidden" name="spec_name_1" value="' + SPEC.spec_name_1 + '" />');
    }else{
        spec_name_1.hide();
        spec_name_1.html('');
    }
    if(SPEC.spec_name_2){
        spec_name_2.show();
        spec_name_2.html(SPEC.spec_name_2 + '<input type="hidden" name="spec_name_2" value="' + SPEC.spec_name_2 + '" />');
    }else{
        spec_name_2.hide();
        spec_name_2.html('');
    }
    ...
    SPEC.specs && $.each(SPEC.specs,function(i,item){
        ...
        if(SPEC.spec_name_1){
            tpl.find('*[item="spec_1"]').append('<input type="hidden" name="spec_1[]" value="' + item.spec_1 + '" />' + item.spec_1);
        }else{
            tpl.find('*[item="spec_1"]').append('<input type="hidden" name="spec_1[]" value="" />');
            (SPEC.spec_qty == "1" || SPEC.spec_qty == "0") && tpl.find('*[item="spec_1"]').hide();
        }
        if(SPEC.spec_name_2){
            tpl.find('*[item="spec_2"]').append('<input type="hidden" name="spec_2[]" value="' + item.spec_2 + '" />' + item.spec_2);
        }else{
            tpl.find('*[item="spec_2"]').append('<input type="hidden" name="spec_2[]" value="" />');
            (SPEC.spec_qty == "1" || SPEC.spec_qty == "0") && tpl.find('*[item="spec_2"]').hide();
        }
        tpl.find('*[item="price"]').append('<input type="hidden" name="price[]" value="' + item.price + '" />' + item.price);
        tpl.find('*[item="price2"]').append('<input type="hidden" name="price2[]" value="' + item.price2 + '" />' + item.price2);
        tpl.find('*[item="stock"]').append('<input type="hidden" name="stock[]" value="' + item.stock + '" />' + item.stock);
        tpl.find('*[item="sku"]').append('<input type="hidden" name="sku[]" value="' + item.sku + '" />' + item.sku);
        tpl.show();
...
function spec_editor(){
    ...
    spec_item && $.each(spec_item,function(i,item){ // 遍历生成规格项
        ...
        tpl.find('*[item="price2"]').val(item.price2);
    ...
    $('*[ectype="add_spec_item"]').click(function(){
        $('#dialog_object_spec_editor').find('*[ectype="data"]:last').clone(true).insertBefore($('#dialog_object_spec_editor').find('*[ectype="data"]:last'));
        hide_drop_button();
    });
    ...
    $('*[ectype="save_spec"]').click(function(){
        ...
        $('#dialog_object_spec_editor').find('*[ectype="data"]').each(function(){
            ...
            var price2 = $.trim($(this).find('*[item="price2"]').val());
            ...
            if(SPEC.spec_qty == 1){ // 一个规格
                ...
                eval('if(spec_' + spec_pos + ' || (!spec_' + spec_pos + ' && !price && !stock && !sku && !price2)){}else{complate = false;}');
            }else{ // 两个规格
                if((spec_1 && spec_2) || (!spec_1 && !spec_2 && !price && !stock && !sku && !price2)){
