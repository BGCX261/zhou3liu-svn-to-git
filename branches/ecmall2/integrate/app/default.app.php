...
class DefaultApp extends IntegraterApp
{
    ...
    function config()
    {
        ...
        $this->assign('data', array(
            ...
            'app_url'     => isset($_POST['app_url']) ? trim($_POST['app_url']) : dirname(site_url()),
    ...
    function config_done()
    {
        ...
        if (!eregi('^http(s?)://', $uc_info['uc_api']))
        {
            $this->_ierror(array(
                'error' => array(
                    'key'     => 'uc_api',
        ...
        if (!eregi('^http(s?)://', $uc_info['app_url']))
        {
            $this->_ierror(array(
                'error' => array(
                    'key'     => 'app_url',
        ...
        if (!$_recheck)
        {
            ...
            if (!empty($tmp))
            {
                ...
                if (count($arr) > 1)
                {
                    ...
                    {
                        $error = array('content' => sprintf(Lang::get('get_ucinfo_failed'), CHARSET, $uccharset));
                    }
                    /* 检测UC客户端版本 */
