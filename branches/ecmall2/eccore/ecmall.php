...
class ECMall
{
    ...
    function startup($config = array())
    {
        ...
        $app    = isset($_REQUEST['app']) ? strtolower(trim($_REQUEST['app'])) : $default_app;
...
//    if(!$has_login)
//    {
//        $price="<a href='" . site_url() . "/index.php?app=member&act=login&ret_url='>view price</a>";
//        return $price;
//    }
...
/** 
 * LLL 返回当前语言的2个字母语言代称部分
 * 
 * @param void
 * @return string, as 'fr','sc'....
 */
function get_lang2()
{
    $lang2=explode('-',LANG);
...
/**
 * LLL 截取字符串中指定语言的部分
 * @param $source  形如  ~|en:clothes~|sc:衣~|it:bbbbb3220*dl!
 * @param $lang2 形如sc
 */
function get_part_string($source,$lang2,$show_all=false)
{
    $tempstr=explode('~|',$source);
    foreach($tempstr as $key=>$value)
    {
        if(substr($value,0,2)==$lang2 && $show_all==false)
        {
            return substr($value,3);//如:衣
        }
    }
    //return substr($tempstr['1'],3);//如clothes

    foreach($tempstr as $key=>$value)
    {
        substr($value,2,1)==':' && $tempstr[$key]=substr($value,3);
        if($value=='') {unset($tempstr[$key]);}
    }
    return implode(' | ',$tempstr);//将多种语言字符合并
}
...
function ecm_define($source)
{
    ...
    foreach ($source as $key => $value)
    {
        ...
        {
            /* 如果是可被定义的，则定义 */
            if (strtoupper($key)!='LANG') //LLL 语言不定义
            {
