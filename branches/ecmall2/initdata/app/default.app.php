...
class DefaultApp extends BaseApp
{
    ...
    function DefaultApp()
    {
        ...
        $system_info = include(ROOT_PATH . '/data/system.info.php');
        $version = str_replace(' ', '', strtolower($system_info['version']));
        if (!in_array($version, array('2.0beta3', '2.0beta4', '2.0final')))
        {
            $this->show_message('很抱歉，该测试数据和您的 ECMall 版本不兼容，请您下载合适的测试数据后再安装。');
            return;
        }
